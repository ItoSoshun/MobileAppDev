...
Phase 1 → Phase 2 → ... → Phase 7 の順に実装し、各Phaseごとに動作確認を行います。
...
1. **データベースマイグレーション**: v1 → v2（新テーブル追加）
2. **ファイルコピー**: 既存ファイルを新ストレージにコピー
3. **メタデータ抽出**: ファイル名から日時・タグを抽出してDB登録
4. **検証期間**: 旧データは削除せず、一定期間保持
5. **クリーンアップ**: ユーザー確認後、旧データを削除

### ロールバック対応

- 既存ファイルは削除しない（Phase 7まで保持）
- データベースバックアップを事前作成
- エラー時は旧実装で継続使用可能
